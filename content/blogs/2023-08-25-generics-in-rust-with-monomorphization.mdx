...
So, what are generics? In simple words, they mean parameterized types. The idea is to allow types to be parameters to structs, functions, traits, methods, etc. Let's consider a function that swaps elements in an array:
...
We dove headfirst into the generics implementation, but let's break it down now. In the code above, the function `swap_elements` is defined with a generic type parameter `T`. This parameter allows the function to work with arrays containing elements of any type. Now having this abstraction, one might wonder what the performance hit for this is. The good news is, it's zero. Rust achieves this zero-cost abstraction through a process called `Monomorphization`.
...
One thing to note is that we've used only one generic type to define `Point<T>`. That means `x` and `y` both have to be the same type.
...
fn main() {
    ...
    println!("Original Point: {:?}", int_origin);
    int_origin.swap();
    println!("Swapped Point: {:?}", int_origin);
}

// Original Point: Point { x: 0, y: 1 }
// Swapped Point: Point { x: 1, y: 0 }
```

Why define `<T>` twice? First, we declare the type parameter (`T`) after the `impl` keyword (`impl<T>`), and then we use the other `T` to refer to that first `T`: to specify the struct type we are implementing (`Point<T>`).
