...
description: Zero cost abstraction is a fancy term that simply means that adding high-level constructs like a newtype, traits or generics does not contribute to any runtime cost.
author: Rafi Hasan
date: 2023-07-06
tags:
  - rust
...
Zero-cost abstraction is a fancy term that simply means that adding high-level constructs like a `newtype`, iterators, traits, or [generics](2023-08-25-generics-in-rust-with-monomorphization) does not
...
Here, the `Bar` struct implements the `Foo` trait. But the `Foo` trait never exists at runtime. The `foo` method is
...
easier to understand. But even though we chain an `iterator` and a `filter` adapter before computing the sum, no intermediate arrays are allocated. The compiler optimizes this chain to generate the exact same assembly code as the imperative loop.
