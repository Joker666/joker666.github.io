...
#### Images
Images are a pixel grid is just a matrix of numbers, already a representation. But a raw pixel matrix doesn't have any semantics. CNNs imposes a representation. The representation forces the network to learn local patterns first (edges, textures), then compose them into shapes, then into objects through convolution. Finally it can decide if it's hotdog or not from an image. CNN is a carefully designed representation of what "looking at an image" should mean computationally.
...
#### Language
Words are discrete. There's no derivative of "king". How do you encode language to a model? But if "King" can be mapped to high-dimensional vector space, you can take a derivative of it and suddenly the model can distinguish King from Queen. Then the question "what is king minus man plus woman?" becomes geometry. The result is a representation space where `king - man + woman ≈ queen` falls out naturally. Not because anyone told the model about it, but because a good representation makes structure visible.
