...
In modern programming languages, concurrency has become an unequivocal need. Almost all programming languages today have some method of concurrency. Some have really powerful constructs that can offload work to different threads in the OS, e.g., Java, some mimic this behavior in the same thread, e.g., Ruby.
...
Let's imagine we have an external API call that takes about 100ms to complete. If we have 1000 of these calls, and we call these synchronously, it would take about **100s** to complete
...
Unless we have all the resources on earth, we are limited in the amount of memory and CPU we can allocate at a given time. While the initial stack size of a goroutine is only 2KB, spawning millions of them will quickly consume gigabytes of memory and crash your application. The above solution runs all the tasks concurrently and let's say for a million of these tasks, it can quickly run out of memory and CPU of the machine. Either we have to upgrade our machine or find a different solution.
